...
db.serialize(() => {
  ...
  db.run("CREATE TABLE IF NOT EXISTS contas (id INTEGER PRIMARY KEY, valor REAL, vencimento TEXT, status TEXT)");
});
...
app.get('/categorias-despesa', (req, res) => {
  db.all("SELECT categoria, SUM(valor) AS total FROM despesas GROUP BY categoria", (err, rows) => {
    if (err) return res.status(500).send(err);
    res.json(rows);
  });
});
